...
USE [Supply_Chain Star Schema];
GO

-- إضافة العمود لجدول الـ Product
ALTER TABLE Dim_Product
ADD Category_Sk INT; -- أو VARCHAR(100) حسب نوع البيانات عندك


USE [Supply_Chain Star Schema];
GO

-- تحديث عمود الكاتيجوري في جدول المنتج بناءً على الجدول المصدر
UPDATE p
SET p.Category_Sk = p0.Category_SK
FROM [dbo].[Dim_Product] p
JOIN [dbo].[Dim_Product0] p0 ON p.Product_SK = p0.Product_SK;

-- تأكيد: لو فيه منتجات جديدة مش موجودة في الجدول بتاعك، ضيفها بـ Distinct
INSERT INTO [dbo].[Dim_Product] (Product_SK, Product_Name, Product_Price, Product_Status, Category_Sk)
SELECT DISTINCT p0.Product_SK, p0.Product_Name, p0.Product_Price, p0.Product_Status, p0.Category_SK
FROM [dbo].[Dim_Product0] p0
WHERE NOT EXISTS (SELECT 1 FROM [dbo].[Dim_Product] p WHERE p.Product_SK = p0.Product_SK);